...
import { getLeaderboardAction, getLeaderboardCategoriesAction } from "@/actions/db/leaderboard-actions"
...
export function LeaderboardSection() {
  ...
  const [currentUserRank, setCurrentUserRank] = useState<LeaderboardEntry | undefined>()
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="space-y-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex items-center gap-4 py-3 px-4 border rounded-md">
          ...
          <div className="space-y-2">
            {leaders.length > 0 ? (
              <>
                ...
                {leaders.map((leader) => (
                  <motion.div
                    ...
                    className={cn(
                      "flex items-center gap-4 py-3 px-4 rounded-md transition-colors border",
                      leader.isCurrentUser ? "border-primary" : "border-border hover:bg-accent/10"
                    )}
                  >
                    ...
                    <Avatar className="h-9 w-9">
                      <AvatarImage src={getProfileImage(leader.profileImageUrl) || undefined} alt={leader.displayName} />
                      <AvatarFallback>{getInitials(leader.displayName)}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center text-sm font-medium">
                        <span className="truncate">{getDisplayName(leader)}</span>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {leader.quizzesCompleted} quiz{leader.quizzesCompleted !== 1 ? "zes" : ""}
                      </p>
                    </div>
                ...
                {currentUserRank && !leaders.some(l => l.isCurrentUser) && (
                  <motion.div
                    ...
                    className="flex items-center gap-4 py-3 px-4 rounded-md border border-primary"
                  >
                    <div className="flex items-center justify-center w-8 font-bold">
                    ...
                    <Avatar className="h-9 w-9">
                      <AvatarImage 
                        src={getProfileImage(currentUserRank.profileImageUrl) || undefined} 
                        alt={currentUserRank.displayName} 
                      />
                      <AvatarFallback>{getInitials(currentUserRank.displayName)}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center text-sm font-medium">
                        <span className="truncate">{getDisplayName(currentUserRank)}</span>
                        <Badge variant="outline" className="ml-2 text-xs">
                          You
                        </Badge>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {currentUserRank.quizzesCompleted} quiz{currentUserRank.quizzesCompleted !== 1 ? "zes" : ""}
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-semibold">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {currentUserRank.correctAnswers} / {currentUserRank.totalAttempts}
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="text-center py-8 text-muted-foreground">
                No data available yet. Complete some quizzes to appear on the leaderboard!
              </div>
            )}
          </div>
        )}
